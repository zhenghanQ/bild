...
def get_subjectinfo(subject_id, base_dir, task_id, model_id):
    ...
    if os.path.exists(json_info):
        ...
        if os.path.exists(task_scan_key):
            ...
            TA = np.genfromtxt(task_scan_key)[3] #GAC
        else:
            TR = np.genfromtxt(os.path.join(base_dir, 'scan_key.txt'))[1]
            TA = np.genfromtxt(os.path.join(base_dir, 'scan_key.txt'))[3] #GAC, added TA to return line too
    return run_ids[task_id - 1], conds[task_id - 1], TR, TA
...
def analyze_openfmri_dataset(data_dir, subject=None, model_id=None,
    ...
    wf.connect(subjinfo, 'TA', modelspec, 'time_acquisition') #GAC added
    ...
    #wf.connect([(preproc, fixed_fx, [('outputspec.mask','flameo.mask_file')]),
    ...
    modelfit.inputs.inputspec.bases = {'none': None} #GAC, satra email 2/8/2015
